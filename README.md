...
Features:
 ...
 - The command line options are available in the test builds only. See "-dTestBuild" flag in project options.
...
Known issues in the first version(s):
 ...
 - Various other UI interaction bugs, like:
   ...
   - [May not happen in the latest compiler versions] What appears to be a VirtualTreeView bug, is that in WinInterp window, scanning complex windows, leads to bad internal tree structure. The last component cannot be selected (the tree does not respond to click events on the last item).
   - [May not happen in the latest compiler versions] The tree from "Template Call Tree" window may not always display its scroll bars. The bug appears to comes from VirtualTreeView, when deleting nodes (which is part of generating the template call tree).
...
Other:
 ...
 - A custom exception handler is installed, to prevent unhandled exceptions to crash the application. The exception messages are saved to a text file, which automatically opens in Notepad.
